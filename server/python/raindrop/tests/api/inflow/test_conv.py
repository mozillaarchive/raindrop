...
class TestConvoSimple(APITestCaseWithCorpus):
    ...
    def test_identities_mine(self, iids=None):
        ...
        result = self.call_api("inflow/conversations/identities", ids=iids)
        seen = set()
        for convo in result:
    ...
    def test_direct(self, endpoint="inflow/conversations/direct",
        ...
        result = self.call_api(endpoint, schemas=schemas)
    ...
    def test_personal(self):
        self.test_direct("inflow/conversations/personal")
    ...
    def test_personal_specific_bad_schema(self):
        self.test_direct("inflow/conversations/personal",
                         ['rd.this-doesnt-exist'], False)

    def test_twitter(self):
        result = self.call_api("inflow/conversations/in_groups",
    ...
    def test_twitter_inflow(self):
        ...
        for conv in look:
            self.sanity_check_convo(conv)
            # confirm only one message
            self.failUnlessEqual(1, len(conv['messages']), pformat(conv))

    def test_with_messages(self):
        known_msgs = self.get_known_msgs_to_identities()
        result = self.call_api("inflow/conversations/with_messages",
...
"""
    ...
    def test_message_limit_summary(self):
        result = self.test_message_limit(2)
